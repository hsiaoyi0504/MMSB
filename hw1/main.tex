...
\subsection{Problem Set 2.4.7 Network modelling.}
	\begin{enumerate}[a)]
		...
		\begin{enumerate}[i)]
			...
			It's intutive that the steady-state of a system exists if and only if there is a steady-state concentration of any species. Then, according the steady-state concentration derived in part (ii), the steady-state concentration of A:\\
			...
			and here:\\
			$b_{0}k_{2}=1 \times 1<k_{0}=5$, which is conflict with the sufficient and necessary condition of the steady-state concentration of A, so the system cannot acheive steady-state. 
		\end{enumerate}
